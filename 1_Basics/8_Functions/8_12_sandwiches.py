"""

8-12. Sandwiches: Write a function that accepts a list of items a person wants on a sandwich. 

The function should have one parameter that collects as many items as the function call provides, and it should print a summary of the sandwich that's being ordered. 

Call the function three times, using a different number of arguments each time.

"""

def make_sandwich(*sandwich_items):

    print("Following are the items added to the sandwich: ")
    for item in sorted(sandwich_items):
        print(f"   . {item}")


make_sandwich("item1", "item2", "item4")

make_sandwich("item30", "item40")

make_sandwich("item71", "item72", "item73", "item74", "item78")